...
function buildBot(token, adminTelegramId) {
  ...
  instance.on('text', async (ctx) => {
    ...
    await addLog('info', 'Text received', {
      userId: ctx.from?.id,
      username: ctx.from?.username,
      text,
    });

    await ctx.reply('Я понимаю команды /start и /catalog. Открой каталог командой /catalog.');
  });
...
async function startBotPolling({ token, adminTelegramId }) {
  ...
  // Если бот ранее работал через webhook, polling не стартует без удаления webhook.
