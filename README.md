...
Если бот раньше был настроен на webhook, polling может не стартовать.
В текущей версии при запуске webhook автоматически удаляется, после чего бот начинает принимать сообщения в локальном режиме.
